...
void Rpc<TTr>::pkt_loss_retransmit_st(SSlot *sslot) {
  ...
  if (unlikely(delta == 0)) {
    ERPC_INFO("pkt_loss_retransmit_st1 : %s: False positive. Ignoring.\n", issue_msg);
    return;
  }
  ...
  if (unlikely(sslot->client_info_.wheel_count_ > 0)) {
    pkt_loss_stats_.still_in_wheel_during_retx_++;
    ERPC_INFO("pkt_loss_retransmit_st2 : %s: Packets still in wheel. Ignoring.\n", issue_msg);
    return;
  }
  ...
  ERPC_INFO("pkt_loss_retransmit_st3 : %s: Retransmitting %s.\n", issue_msg,
